    ...
    <>
      ...
      <div className="relative flex min-h-screen flex-col bg-[#F8FAFC] text-[#1b1b18] dark:bg-[#0a0a0a] dark:text-[#EDEDEC] font-['Instrument_Sans']">
        ...
        <header className="sticky top-0 z-20 w-full border-b border-gray-200/70 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:border-gray-800/70 dark:bg-[#0a0a0a]/70">
          ...
          {isMenuOpen && (
            <div className="sm:hidden">
              <div className="fixed inset-0 z-30 bg-black/30 backdrop-blur-[1px]" aria-hidden="true" />
                ...
                <nav className="flex flex-col gap-4">
                  {auth.user && isRegularUser && (
                    <Link
                      ...
                      href={route('library-type-dashboard')}
                      className={`${buttonBase} ${buttonSuccess} w-full text-base shadow-md`}
                    >
                  ...
                  {auth.user && isStaffOrAdmin && (
                    <Link
                      onClick={handleNavigation}
                      href={route('dashboard')}
                      className={`${buttonBase} ${buttonPrimary} w-full shadow-md`}
                    >
                      <LayoutDashboardIcon size={16} />
                      ផ្ទាំងគ្រប់គ្រង
                    </Link>
                  ...
                  {auth.user ? (
                    <Link
                      ...
                      method="post"
                      as="button"
                      className={`${buttonBase} ${buttonDanger} w-full shadow-md`}
                    >
                      ចាកចេញ
                    </Link>
                  ) : (
            ...
            {auth.user ? (
              ...
              <>
                <span className="bg-gradient-to-r from-blue-600 to-emerald-500 bg-clip-text text-transparent dark:from-blue-400 dark:to-emerald-400">
                  បណ្ណាល័យឌីជីថលរបស់សកលវិទ្យាល័យអន្តរជាតិ ឌូវី
                </span>
              </>
            )}
          </h2>
        ...
        <footer className="mt-auto border-t border-gray-200/70 bg-white/40 p-4 text-center text-sm text-gray-600 backdrop-blur dark:border-gray-800/70 dark:bg-[#0a0a0a]/40 dark:text-gray-400 sm:p-6">
          © {new Date().getFullYear()} រក្សាសិទ្ធិគ្រប់យ៉ាងដោយ សកលវិទ្យាល័យអន្តរជាតិ ឌូវី
        </footer>
      </div>
