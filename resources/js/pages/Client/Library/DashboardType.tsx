...
const Link = ({ href, children, className = "", ...props }: LinkProps) => (
  <a
    href={href}
    className={[focusRing, "outline-none", className].join(" ")}
    {...props}
  >
    {children}
  </a>
...
/** ---------------------------------------------------
 *  Tailwind-safe color style tokens
 *  (avoid bg-${color} purge issues)
 *  --------------------------------------------------- */
const colorStyles = {
  cyan: {
    iconWrap:
      "from-cyan-50 to-sky-50 text-cyan-700 ring-cyan-100 dark:from-cyan-950/30 dark:to-sky-950/30 dark:text-cyan-300 dark:ring-cyan-900/40",
    ...
    button:
      "from-cyan-500 to-sky-600 hover:from-cyan-500 hover:to-sky-700 dark:from-cyan-500 dark:to-sky-600",
    glow: "from-cyan-500/20 via-sky-500/20 to-cyan-500/20",
  },
  yellow: {
    iconWrap:
      "from-yellow-50 to-amber-50 text-amber-700 ring-amber-100 dark:from-yellow-950/30 dark:to-amber-950/30 dark:text-amber-300 dark:ring-amber-900/40",
    ...
    button:
      "from-amber-500 to-yellow-600 hover:from-amber-500 hover:to-yellow-700 dark:from-amber-500 dark:to-yellow-600",
    glow: "from-amber-500/20 via-yellow-500/20 to-amber-500/20",
  },
  rose: {
    iconWrap:
      "from-rose-50 to-pink-50 text-rose-700 ring-rose-100 dark:from-rose-950/30 dark:to-pink-950/30 dark:text-rose-300 dark:ring-rose-900/40",
    ...
    button:
      "from-rose-500 to-pink-600 hover:from-rose-500 hover:to-pink-700 dark:from-rose-500 dark:to-pink-600",
...
function LibraryCard({
  ...
  title: string;
  desc: string;
  colorKey: keyof typeof colorStyles;
    ...
    <Link href={href} className="group block">
      <div
        className={[
          ...
          style={{ boxShadow: "0 8px 32px -12px rgba(59,130,246,0.13)" }}
        >
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(1200px 300px at 50% -10%, rgba(59,130,246,0.14), transparent 60%)",
            }}
          />

          <div className="relative z-[1] flex flex-col items-center space-y-6 text-center">
            {/* Icon capsule */}
            <div
              className={[
                "rounded-2xl p-6 ring-2 ring-inset",
                "bg-gradient-to-br",
                c.iconWrap,
                c.iconWrapHover,
                motionSafe,
            ...
            >
              <div className="transition-transform duration-700 group-hover:rotate-3 group-hover:scale-110">
                {icon}
              </div>
            </div>
            ...
            <h3
              className={[
                "text-2xl font-extrabold tracking-tight text-gray-900 dark:text-white drop-shadow-sm",
            ...
            {/* Description */}
            <p className="max-w-[32ch] text-balance text-gray-700 dark:text-gray-300 text-base opacity-90">
              {desc}
            </p>
            ...
            <div
              className={[
                "mt-2 h-1 w-40 rounded-full bg-gradient-to-r blur-[2px] opacity-80",
                c.glow,
              ].join(" ")}
            />
            ...
            <Button
              className={[
                "mt-8 shadow-lg hover:shadow-xl",
                "bg-gradient-to-r",
                c.button,
          ...
          <div
            aria-hidden
            className="pointer-events-none absolute inset-0 rounded-3xl ring-2 ring-transparent transition-all duration-500 group-hover:ring-blue-300/60 dark:group-hover:ring-blue-900/40"
          />
        </div>
      </Link>
    );
...
/** ---------------------------------------------------
 *  Page: Layered background (gradient + grid + aura)
 *  --------------------------------------------------- */
const DashboardType = () => {
  const cards = [
    // {
    //   title: "បណ្ណាល័យសកល",
    //   desc: "ស្វែងរកសៀវភៅទាំងអស់នៅគ្រប់សាខា",
    //   colorKey: "cyan" as const,
    //   icon: <Globe className="h-10 w-10" />,
    //   route: route("global library"),
    // },
    {
      title: "បណ្ណាល័យរូបវន្ត",
      desc: "ស្វែងរកសៀវភៅនៅក្នុងបណ្ណាល័យរបស់យើង",
      colorKey: "yellow" as const,
      icon: <Library className="h-10 w-10" />,
      route: route("local library"),
    },
    {
      title: "បណ្ណាល័យអេឡិចត្រូនិក",
      desc: "សៀវភៅឌីជីថលដែលអាចអានបានភ្លាមៗ",
      colorKey: "rose" as const,
      icon: <BookOpen className="h-10 w-10" />,
      route: route("global e-library"),
    },
  ];
  ...
  return (
    <div
      className={[
        ...
        "bg-gradient-to-br from-fuchsia-200 via-sky-100 to-lime-100",
        "dark:from-slate-950 dark:via-indigo-950 dark:to-blue-950",
    ...
    >
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        ...
        />
        <div
          className="absolute inset-0 hidden opacity-[0.16] dark:block [mask-image:radial-gradient(ellipse_at_center,black,transparent_70%)]"
          style={{
            backgroundImage:
      ...
      <div className="container mx-auto px-4 text-center">
        ...
        <a
          href="/"
          className="mb-10 inline-block transition-transform duration-300 hover:scale-110 drop-shadow-2xl"
          aria-label="ទៅកាន់ទំព័រដើម"
        >
          <img
            src="/images/DIU.png"
            alt="DIS Library Logo"
            className="h-32 w-auto drop-shadow-2xl sm:h-36"
          />
        </a>
        ...
        <h1
          className={[
            "mb-4 text-4xl font-extrabold leading-25 tracking-tight sm:text-5xl md:text-6xl",
            "bg-[linear-gradient(90deg,#2563eb_0%,#22c55e_50%,#a855f7_100%)] bg-clip-text text-transparent",
            "drop-shadow-[0_4px_24px_rgba(59,130,246,.18)] dark:drop-shadow-[0_6px_26px_rgba(59,130,246,.25)]",
          ].join(" ")}
        >
         បណ្ណាល័យឌីជីថលរបស់សាកលវិទ្យាល័យអន្តរជាតិ ឌូវី
        </h1>

        {/* Subtitle */}
        {/* <p className="mx-auto mb-12 max-w-2xl text-lg text-gray-700 dark:text-gray-300 opacity-90">
          ជ្រើសរើសប្រភេទបណ្ណាល័យដែលអ្នកចង់ស្វែងយល់បន្ថែម
        </p> */}

        {/* Grid */}
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-10 md:grid-cols-2">
          {cards.map((card, i) => (
            <LibraryCard
              key={i}
              title={card.title}
