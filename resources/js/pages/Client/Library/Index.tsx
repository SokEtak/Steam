...
interface Book {
    ...
    published_at?: string | number; // Updated to allow number for int4 year
    created_at?: string;
...
// Utility to format date or year
const formatDate = (dateInput: string | number | undefined): string => {
    if (!dateInput) return 'Unknown';
    if (typeof dateInput === 'number') {
        // Handle int4 year (e.g., 2023)
        return dateInput.toString(); // Return the year as a string
    }
    try {
        // Handle date string (e.g., "2023-10-01")
            ...
            if (sortBy === "Newest") {
                ...
                if (dateA && dateB) {
                    if (typeof dateA === 'number' && typeof dateB === 'number') {
                        // Compare years directly if both are numbers
                        return dateB - dateA;
                    }
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-4 max-w-8xl mx-auto justify-center px-2">
                    ...
                    <Select value={sortBy} onValueChange={setSortBy}>
                        ...
                        <SelectContent className="bg-white border-gray-200 text-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                            ...
                            <SelectItem value="Newest" className="hover:bg-gray-100 dark:hover:bg-gray-700">ថ្មីបំផុត</SelectItem>
                            {/*<SelectItem value="Title A-Z" className="hover:bg-gray-100 dark:hover:bg-gray-700">Title (A-Z)</SelectItem>*/}
                            <SelectItem value="Title A-Z" className="hover:bg-gray-100 dark:hover:bg-gray-700">ចំណងជើង (ក-អ)</SelectItem>
                            <SelectItem value="Most Viewed" className="hover:bg-gray-100 dark:hover:bg-gray-700">ពេញនិយម</SelectItem>
                    ...
                    {[
                        ...
                        <Select key={label} value={value} onValueChange={onChange}>
                            <SelectTrigger className={`w-full sm:w-40 bg-white border border-gray-300 text-gray-900 hover:border-gray-400
                                                    dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:border-gray-600
                                                    focus:ring-${accentColor}-500 transition rounded-full`}>
                                <SelectValue placeholder={label} />
                            </SelectTrigger>
                            <SelectContent className="bg-white border-gray-200 text-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
                                <SelectItem value="All" className="hover:bg-gray-100 dark:hover:bg-gray-700">{label}ទាំងអស់</SelectItem>
                                {options.map((opt) => (
                                                ...
                                                <span className="text-xs text-gray-400 dark:text-gray-500 truncate font-medium flex items-center min-w-0">
                                                    ...
                                                    {isContributorVerified && (
                                                        <Tooltip>
                                                            ...
                                                            <TooltipContent>Verified Contributor</TooltipContent>
                                                        </Tooltip>
                                                    )}
                                                </span>
                                            </div>
                                        ...
                                        >
                                            <div className="space-y-4">
                                                ...
                                                <div className="text-sm text-gray-500 dark:text-gray-400 space-y-1.5 pt-3 border-t border-gray-200 dark:border-gray-600">
                                                    ...
                                                    {/*english version*/}
                                                    {/*{book.language && <p><span className="font-medium text-gray-700 dark:text-gray-300">ភាសា:</span> {book.language === 'en' ? 'English' : book.language === 'kh' ? 'Khmer' : book.language}</p>}*/}
                                                    ...
                                                    {book.bookcase?.code && <p><span className="font-medium text-gray-700 dark:text-gray-300">ទីតាំងរបស់សៀវភៅ:</span> {book.bookcase.code} / {book.shelf?.code}</p>}
