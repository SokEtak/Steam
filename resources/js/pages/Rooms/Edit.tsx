...
interface Room {
    ...
    room_number: string;
    floor: number;
    building: { id: number; name: string; campus_id: number; campus: { name: string } };
...
export default function RoomsEdit({
    ...
    const handleCancel = () => {
        ...
        else router.visit(route("rooms.show", room.id));
    };

    const breadcrumbs = [
        { title: t.indexTitle, href: route("rooms.index") },
        { title: t.showTitle, href: route("rooms.show", room.id) },
                ...
                <div className="max-w-1xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <label><span className="text-red-500">*</span> {t.editCampus}</label>
                            <Popover open={openCampus} onOpenChange={setOpenCampus}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className="w-full justify-between">
                                        {selectedCampusName ?? t.editCampusPlaceholder}
                                        <ChevronDown className="ml-2 h-4 w-4 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="p-0 w-full" align="start">
                                    <Command>
                                        <CommandInput placeholder={t.editCampusPlaceholder} />
                                        <CommandList>
                                            <CommandEmpty>{t.editCampusEmpty ?? "No campus found."}</CommandEmpty>
                                            <CommandGroup>
                                                {campuses.map(c => (
                                                    <CommandItem
                                                        key={c.id}
                        ...
                        {/* Building */}
                        <div className="space-y-2">
                            <label><span className="text-red-500">*</span> {t.editBuilding}</label>
                            <Popover open={openBuilding} onOpenChange={setOpenBuilding}>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        {selectedBuildingName ?? (data.campus_id === "none" ? t.editBuildingAll : t.editBuildingPlaceholder)}
                                        <ChevronDown className="ml-2 h-4 w-4 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="p-0 w-full" align="start">
                                    <Command>
                                        <CommandInput placeholder={t.editBuildingPlaceholder} />
                                        <CommandList>
                                            <CommandEmpty>
                                                {filteredBuildings.length === 0
                                                    ? t.editBuildingEmpty
                                                    : t.editBuildingNoMatch}
                                            </CommandEmpty>
                                            <CommandGroup>
                        ...
                        {/* Room Number */}
                        <div className="space-y-2">
                            <label><span className="text-red-500">*</span> {t.editRoomNumber}</label>
                            <Input
                        ...
                        {/* Floor */}
                        <div className="space-y-2">
                            <label><span className="text-red-500">*</span> {t.editFloor}</label>
                            <Input
                                type="number"
                                value={data.floor}
                    ...
                    <AlertDialog open={showLeaveDialog} onOpenChange={setShowLeaveDialog}>
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => router.visit(route("rooms.show", room.id))}>
